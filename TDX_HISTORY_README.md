...
```powershell
python .\sync_tdx_history.py
```

只更新指定代码：

```powershell
python .\sync_tdx_history.py --symbols 600519.SH 510300.SH
```

工作日 16:30 前运行时，程序默认不写入尚未收盘的当日日线。如确实需要包含当天，可显式执行：

```powershell
python .\sync_tdx_history.py --include-today
```

指定其他配置或数据库：

```powershell
python .\sync_tdx_history.py --config .\config\tdx_history.json --database .\data\my_history.sqlite3
```
