...
from daily_report.data_sources.offerings import collect_offerings, normalize_tdx_offerings
...
def test_generate_report_persists_offering_calendar(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "daily_report.service.fetch_a_share_data",
        lambda _universe, _caller: ([], [], [], [], breadth),
    )
    monkeypatch.setattr("daily_report.service.fetch_us_daily", lambda _names: ([], []))
    monkeypatch.setattr("daily_report.service.fetch_crypto_quotes", lambda _names: ([], []))
    monkeypatch.setattr(
        "daily_report.service.collect_offerings",
